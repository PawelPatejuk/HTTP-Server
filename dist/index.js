...
async function createChirpHandler(req, res) {
...
async function getAllChirpsHandler(req, res) {
...
async function getChirpHandler(req, res) {
...
async function loginHandler(req, res) {
    ...
    const flag = await checkPasswordHash(password, hashedPassword);
    if (flag) {
        const token = makeJWT(user.id, expiresInSeconds, apiConfig.secret);
        res.status(200).send({
            "id": user.id,
...
async function deleteChirpHandler(req, res) {
    const chirpId = req.params["chirpID"];
